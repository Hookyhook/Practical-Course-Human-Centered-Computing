"""
Pipeline 1 — MulticlaimNet  (Claim Clustering / Deduplication)
==============================================================
Corpus:    data/raw/MultiClaim.csv  (full raw claim pool)
Baseline:  data/processed/multiclaimnet_preprocessed.csv  (unperturbed queries)
Perturbed: data/processed/multiclaimnet_verified.csv      (verified perturbations)

Two-phase evaluation (H200 / CUDA only):
  Phase 1 — embed full preprocessed baseline, compute MAP@k + MAP@k_rr
  Phase 2 — embed verified perturbed queries, merge with baseline metrics

Task:
  Given a perturbed claim, retrieve OTHER claims with the same ClusterID
  (query's own NID excluded from results and relevant set).

Usage (from repo root):
  python embeddings/pipeline_multiclaimnet.py --models me5,bge-m3,labse
...
from pipeline_utils import (
    ...
    print_delta_table, cluster_sets_excluding_self, filter_self_from_retrieved,
    filter_verified_queries, add_performance_args, add_reranker_args,
...
OUTPUT_DIR = Path("results/multiclaimnet")

CORPUS_FILE    = DATA_DIR / "raw" / "MultiClaim.csv"
BASELINE_FILE  = DATA_DIR / "processed" / "multiclaimnet_preprocessed.csv"
PERTURBED_FILE = DATA_DIR / "processed" / "multiclaimnet_verified.csv"


def _load_corpus():
    corpus_df = pd.read_csv(CORPUS_FILE, dtype=str).fillna("")
    corpus_texts = corpus_df["Claim"].tolist()
    corpus_nid_map = {nid: i for i, nid in enumerate(corpus_df["NID"].tolist())}
    cluster_to_idxs: dict[str, set] = defaultdict(set)
    for i, cid in enumerate(corpus_df["ClusterID"].tolist()):
        cluster_to_idxs[cid].add(i)
    return corpus_df, corpus_texts, corpus_nid_map, cluster_to_idxs


def _build_qrels(queries_df: pd.DataFrame, corpus_nid_map: dict, cluster_to_idxs: dict):
    gold = [corpus_nid_map.get(nid) for nid in queries_df["NID"]]
    cluster_sets = cluster_sets_excluding_self(
        [cluster_to_idxs.get(cid, set()) for cid in queries_df["ClusterID"]],
        gold,
    )
    return gold, cluster_sets
...
def run(
    ...
    print("\n" + "=" * 65)
    print("PIPELINE: MulticlaimNet — Claim Clustering (two-phase)")
    print("Corpus:   MultiClaim.csv (raw, full pool)")
    print(f"Baseline: {BASELINE_FILE.name}")
    print(f"Perturbed:{PERTURBED_FILE.name}")
    print("Task:     perturbed claim → retrieve other claims from same ClusterID")
    print("Metric:   MAP@k (multi-relevant, cluster denominator)")
    ...
    print("\nLoading corpus...")
    _, corpus_texts, corpus_nid_map, cluster_to_idxs = _load_corpus()
    cluster_sizes = [len(v) for v in cluster_to_idxs.values()]
    print(f"  {len(corpus_texts):,} claims  |  {len(cluster_to_idxs):,} clusters")
    ...
    baseline_gold, baseline_clusters = _build_qrels(baseline_df, corpus_nid_map, cluster_to_idxs)
    baseline_texts = baseline_df["Claim"].tolist()
    ...
    pert_gold, pert_clusters = _build_qrels(perturbed_df, corpus_nid_map, cluster_to_idxs)
    ...
    for model_key in model_keys:
        ...
        corpus_vecs = load_or_embed(
            CACHE_DIR / f"mcnet_{model_key}_multiclaim_raw_corpus.npy",
            corpus_texts, model, prefix=spec.passage_prefix, batch_size=model_batch)
        # Corpus embedding is the biggest single encode call — hand any
        # fragmented/cached blocks back before building the FAISS index,
        # rather than letting them accumulate silently until a later phase OOMs.
        ...
        baseline_vecs = load_or_embed(
            CACHE_DIR / f"mcnet_{model_key}_baseline_queries.npy",
            baseline_texts, model, prefix=spec.query_prefix, batch_size=model_batch)

        baseline_retrieved = faiss_search(index, baseline_vecs, top_k + 1)
        baseline_retrieved = filter_self_from_retrieved(
            baseline_retrieved, baseline_gold, top_k)
        ...
        # encode + search calls back to the allocator before Phase 2.
        ...
        pert_vecs = load_or_embed(
            CACHE_DIR / f"mcnet_{model_key}_pert_queries.npy",
            pert_texts, model, prefix=spec.query_prefix, batch_size=model_batch)

        pert_retrieved = faiss_search(index, pert_vecs, top_k + 1)
        pert_retrieved = filter_self_from_retrieved(pert_retrieved, pert_gold, top_k)

        # Free bi-encoder + FAISS before cross-encoder (largest VRAM spike).
        ...
        if reranker_spec:
            baseline_rr = rerank_candidates(
                ...
                cache_path=CACHE_DIR / f"mcnet_{model_key}_{reranker_key}_baseline_rr.npy",
        ...
        # Paired baseline: NID → position in baseline arrays
        baseline_nid_to_pos = {str(nid): pos
                               for pos, nid in enumerate(baseline_df["NID"])}
        ...
        if reranker_spec:
            pert_rr = rerank_candidates(
                ...
                reranker_spec=reranker_spec,
                batch_size=reranker_spec.batch_size or perf.reranker_batch_size,
                cache_path=CACHE_DIR / f"mcnet_{model_key}_{reranker_key}_pert_rr.npy",
        ...
        print_delta_table(pd.DataFrame(rows), model_key, "MulticlaimNet")

        # Incremental save per model
        df_partial = pd.DataFrame(all_rows)
        df_partial.to_csv(OUTPUT_DIR / "multiclaimnet_results.csv", index=False)
        release_gpu_memory(label=f"end of model [{model_key}]")

    df_out = pd.DataFrame(all_rows)
    out_path = OUTPUT_DIR / "multiclaimnet_results.csv"
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="MulticlaimNet — Claim Clustering Pipeline (H200)")
