...
# ── config ────────────────────────────────────────────────────────────────────
# LM Studio server settings (formerly Pertubator.preset.json in LM Studio UI).
...
# enableThinking=false (Qwen3.5)
LLM_EXTRA_BODY = {
    "chat_template_kwargs": {"enable_thinking": False},
}

# llm.prediction.structured from Pertubator preset
PERTURBATOR_RESPONSE_SCHEMA = {
    "type": "object",
    "properties": {
        "perturbed_text": {"type": "string"},
    },
    "required": ["perturbed_text"],
    "additionalProperties": False,
}

PERTURBATOR_RESPONSE_FORMAT = {
    "type": "json_schema",
    "json_schema": {
        "name": "perturbator_response",
        "strict": True,
        "schema": PERTURBATOR_RESPONSE_SCHEMA,
    },
}
...
def _llm(instruction: str, text: str) -> str:
    ...
    resp = client.chat.completions.create(
        ...
        response_format=PERTURBATOR_RESPONSE_FORMAT,
        extra_body=LLM_EXTRA_BODY,
    )
    msg = resp.choices[0].message
    raw = msg.content or getattr(msg, "reasoning_content", "") or ""
    return json.loads(raw.strip())["perturbed_text"]
